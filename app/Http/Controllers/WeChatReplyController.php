...
class WeChatReplyController extends Controller
{
    function set()
    {
        ...
        WeChatReply::updateOrCreate(['wechat_id' => $data['wechat_id'], 'keyword' => '公众号授权第三方平台-早起提醒'], $data);
        return $this->created();
    }
    ...
    function get()
    {
        ...
        $reply = WeChatReply::where(['wechat_id' => $wechat_id, 'keyword' => '公众号授权第三方平台-早起提醒'])->first();
        if (!$reply) {
            return $this->errorNotFound();
        }
    ...
    function restore()
    {
        ...
        useRemainSettingInfoForThirdGzh($appid);
        return $this->created();
    }
}